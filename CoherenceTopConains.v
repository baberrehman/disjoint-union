...
Inductive contains : typ -> typ -> Prop :=
  | containsSelf : forall A, contains A A
  | containsUnion1 : forall A B C, contains A B -> contains A (OrT B C)
  | containsUnion2 : forall A B C, contains A C -> contains A (OrT B C)
  | containsInter: forall A B C, contains A B -> contains A C -> contains A (AndT B C).

(* Disjointness: Specification *)

Definition OrthoS (A B: typ) := forall C, sub A C -> sub B C -> 
                                  (contains A C /\ contains B C) \/ TopLike C.
...
destruct H.
split; auto.
specialize (H (FunT C1 C2)).
destruct H; eauto.
left.
destruct H.
split; auto.
specialize (H (AndT C1 C2)).
destruct H; eauto.
destruct H.
left.
split; auto.
specialize (H (OrT C1 C2)).
destruct H; eauto.
destruct H.
left.
split; auto.
...
unfold OrthoS in H.
assert (t0: ( contains IntT IntT /\ contains IntT IntT \/ TopLike IntT)); auto.
specialize (H IntT).
assert (t0: ( contains IntT IntT /\ contains IntT IntT \/ TopLike IntT)); auto.
destruct t0. destruct H0.
...
right; apply completeTopLike in H0; auto.
right; apply completeTopLike in H0; auto.
(*Case TopT t1*)
unfold OrthoS. intros.
induction C; eauto; right; apply completeTopLike in H; auto.
