...
Definition DisjSpec A B := forall C, 
(C <: A /\ C <: B -> bottomlike C)
\/ (not (A <: B) /\ not (B <: A) /\ Isomorphic C (t_and A B)).
...
Proof.
intros. unfold btmLikeSpec. inductions H; eauto.
Qed.
...
intros. inductions H; unfold DisjSpec; intros; eauto.
- left. intros. destruct H. apply btm_sub_btmlike in H0. auto.
- left. intros. destruct H. apply btm_sub_btmlike in H. auto.
- left. intros. destruct H.
  inductions C; eauto; try solve [inversion H]; try solve [inversion H0].
  + apply sub_or in H. destruct H.
...
- left. intros. destruct H.
  ...
  + admit. 
- unfold DisjSpec in *.
  specialize (IHdisjointness1 C0).
  specialize (IHdisjointness2 C0).
  destruct IHdisjointness1; destruct IHdisjointness2.
  left. intros. destruct H3. admit. admit. admit. admit.
- admit.
- unfold DisjSpec in *.
  specialize (IHdisjointness C0).
  destruct IHdisjointness.
  left. intros. apply H0. destruct H1.
  apply sub_and in H1. destruct~ H1.
  destruct H0. destruct H1.
  right. split. unfold not. intros.
  unfold not in *.
  apply H0.
  unfold Isomorphic in H2. destruct H2.
  admit. admit.
- admit.
- admit.
- admit.    
...
Lemma BL_disj : forall A, bottomlike A -> forall B, A *a B. 
  induction 1; intros; eauto. admit. admit.
Admitted.
