...
(*
Update started on February 24, 2021
*)

(*
This file contains the updates suggested by Bruno with Bool primitive type
...
Fixpoint FindSubtypes (A: typ) :=
    match A with
    | t_top         => [t_int; t_bool; t_arrow t_top t_bot; t_top]
    | t_bot         => []
    | t_int         => [t_int]
    | t_bool         => [t_bool]
...
Proof.
 ...
 - apply arrow_in_top_bot in H1'. inverts H1'.
   right. right. right. apply elem_append_in_list.
Defined.
...
Proof.
  ...
  - lets H': H.
    ...
    simpl. destruct H'. auto. auto. auto.
...
Proof.
  ...
  unfold DisjAlgo1.
  eapply not_in5.
  apply demorgan4. unfold not. intros.
  destruct H0 as [H1 | [H1 | [H1 | H1]]];
  lets H1': H1;
  apply elem_in_findsubtypes_sub in H1';
  lets H1'': H1;
