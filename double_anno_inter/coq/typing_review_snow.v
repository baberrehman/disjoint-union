...
Require Import syntax_wvalue_no_gen_sub.

(*

This file is created on June 15, 2021

June 15, 2021:
---------------
-> extended from typing_wvalue_no_gen_sub.v
-> addressing reviews
-> introduction rule for intersections
...
Lemma chk_inf_2 : forall G e A B,
    typing G (e_ann e A) check B -> typing G (e_ann e A) infer A.
Admitted.
...
Lemma pexpr_chk_inf : forall G e dir A,
...
Admitted. (* relies on the uniqueness of typing on pre-values *)
...
Lemma check_pexpr_ann : forall E p A C,
   ...
   typing E p check A.
Proof.
Admitted. (* via the pexpr_chk_sub lemma *)
...
Proof.
  ...
  - (* app *)
     ...
     * inverts Typ1.
      ...
      (* prove p:A1 checks *)
        ...
        {
            forwards*: typ_inter H6 H7.
            inverts H.
            apply check_pexpr_ann in H8; auto.
            eapply pexpr_chk_sub in H8; eauto.
            assert (typing G (e_ann p A1) infer A1); auto.
            lets*: typing_through_subst_ee.
            forwards*: H10 H.
            unfold changeanno.
            rewrite* (@subst_ee_intro x).
  ...
  - (* typeof *)
    inverts keep Red; eauto.
    + (* open e1 *)
      pick_fresh y.
      assert ( Heq: open_exp_wrt_exp e1 (e_ann p A) = subst_exp (e_ann p A) y (e1 open_ee_var y) ). {
        rewrite <- subst_ee_intro. reflexivity. eauto.
        admit. (* should hold: lc_exp *)
      } rewrite Heq.
      assert ( Heq2: G & y ~: A = G & y ~: A & empty). {
        rewrite* concat_empty_r.
      }
      assert ( Heq3: G = G & empty) by rewrite* concat_empty_r.
      rewrite Heq3.
      applys typing_through_subst_ee. rewrite <- Heq2.
      applys~ H.
      assert (HL: forall p C A D D',  findtype p C -> C <: A -> typing G (e_ann p D) check D' -> typing G (e_ann p A) infer A) by admit.
      (* a lemma that should hold *)
      applys~ HL H12 Typ.
    + admit. (* similar proof *)
  - forwards*: IHTyp1.
Admitted.
...
Below lemmas not used in theorems
...
(*case int*)
 ...
 - destruct* IHTyp.
  ...
  + destruct H. inductions Typ.
     { clear IHTyp. inverts* Typ.
