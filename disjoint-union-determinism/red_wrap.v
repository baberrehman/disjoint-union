...
| tred_arrow : forall e A1 A2 B1 B2,
    ...
    sub A1 B1 ->
    sub B2 A2 ->
    typ_red (trm_abs B1 e B2) (typ_arrow A1 A2) (trm_abs B1 e B2)
...
Inductive red : trm -> trm -> Prop :=
  ...
  | red_abs : forall A e v v' B,
      ...
      value v ->
      typ_red v A v' ->
      red (trm_app (trm_abs A e B) v) (trm_anno (open_ee e (trm_anno v' A)) B)
...
assert (x \notin L) by auto.
forwards * : H1.
assert (typing (E & x ~: B1) (e open_ee_var x) chk A2).
eapply typing_chk_sub; eauto.
admit.
destruct~ IHtyp_red.
exists x.
...
Lemma inf_typ_red_sub : forall E v v' A B,
value v -> typing E v inf A -> typ_red v B v' -> sub A B.
Proof.
intros.
...
Lemma typ_red_typ_det : forall E v v1 v2 A B,
value v -> typing E v inf A -> typing E v inf B -> typ_red v A v1  -> 
typ_red v B v2  -> A = B.
Proof.
intros.
induction H; inverts* H0; inverts* H1.
...
Lemma typ_red_inf_exist : forall E v v1 B, 
value v -> typing E v inf B -> typ_red v B v1   -> 
exists A, typing E v1 inf A /\ typ_red v A v1.
Proof.
intros.
inductions H1.
- exists* (typ_int).
...
induction Red1; introv Typ Red2.
 ...
 - inductions Red2. 
  ...
  + (*need type reduction determinism here*)
    ...
    lets : typing_sub_inf H0 H13.
    destruct H7. destruct H7.
    lets : typ_red_determinism5 H0 H7 H4 H1.
    rewrite H10. auto. 
 - inverts* Red2.
 ...
 - inverts* Red2.
  ...
  + (*need type reduction determinism here*)
   ...
   lets : typing_regular Typ.
   destruct H1.
   lets : typing_sub_inf H Typ.
   destruct H4. destruct H4.
    forwards * : typ_red_determinism5 H H4 H0 H5.
 - inverts* Red2.
  + inverts* Typ. inverts* H0.
    forwards * : IHRed1.
    rewrite H0. auto.
 ...
 - lets Red2': Red2.
  ...
  + (*need type reduction determinism here*)
    ...
    inversion Typ; subst.
    inversion H4.
    lets : typ_red_chk1 H0 H2 H12 H1.
    lets : typing_sub_inf H0 H4.
    destruct H5. destruct H5.
    forwards * : typ_red_determinism5 H0 H5 H1 H10.
    rewrite H7. auto.
  + lets Typ': Typ.
 ...
 - inverts* Red2.
  ...
  + (*need type reduction determinism here*)
    ...
    lets : typing_regular Typ.
    destruct H5.
    lets : typing_sub_inf H0 Typ.
    destruct H7. destruct H7.
    forwards * : typ_red_determinism5 H0 H7 H1 H10.
    rewrite H12. auto.
Qed.
...
Proof.
  ...
  - dependent destruction Red.
   ...
   + inverts Typ1.
     ...
     forwards*  : typing_through_subst_ee_inf. admit.
     destruct H8. destruct H8.
  ...
  - dependent destruction Red.
   + forwards * : IHTyp.
    ...
    eapply typing_chk_sub; eauto.
   + eapply typ_red_chk_prev1 with (B:=T) (E:=E) in H; eauto.
     apply typing_regular in Typ.
     destruct~ Typ. 
  - dependent destruction Red.
   + forwards * : IHTyp.
   ...
   + forwards~ Val : typ_red_val H6.
     ...
     assert (E = E&empty).
     rewrite concat_empty_r. reflexivity.
     rewrite H14. auto. admit.
     apply value_regular in Val. auto. admit.
   + forwards~ Val : typ_red_val H6.
     lets Typ2': Typ.
     apply typing_regular in Typ2'. destruct Typ2'.
     forwards * : typ_red_chk_prev1 Typ H6.
  ...
  (* case: app *)
   - right. destruct* IHTyp1 as [Val1 | [e1' Rede1']].
    + destruct* IHTyp2 as [Val2 | [e2' Rede2']].
      ...
      destruct  Typ2.
      exists* (trm_anno (open_ee e3 (trm_anno x S)) V1). apply red_abs.
  ...
  - destruct* IHTyp.
    + right*.
      ...
      * apply typ_red_val_dir in H5; auto.
        destruct H5.
        exists (open_ee e1 (trm_anno x T1)).
        apply red_typeofv1; eauto.
        apply typing_regular in Typ'.
        destruct~ Typ'.
      * apply typ_red_val_dir in H5; auto.
        destruct H5.
        exists (open_ee e2 (trm_anno x T2)).
