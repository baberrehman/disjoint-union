...
Lemma typing_through_subst_ee_inf1 : forall E F x T e u dir U,
   typing (E & x ~: U & F) e dir T ->
   typing E u inf U ->
...
- forwards * : IHTypT.
  eapply typing_sub; eauto.
- forwards * : IHTypT.
  apply typing_anno. auto.
- forwards * : IHTypT.
  apply_fresh* typing_typeof as y.
  + lets: H0 y.
    assert (y \notin L) by eauto.
    specialize (H5 H6 E(F&y ~: T1) x U).
    forwards * : H5.
    rewrite concat_assoc. reflexivity.
    rewrite concat_assoc. auto.
...
Definition preservation1 := forall E e e' dir T,
  typing E e dir T -> 
  red e e' -> 
  typing E e' dir T.

Lemma preservation_result1 : preservation1.
Proof.
  unfold preservation1.
  introv Typ. gen e'.
  lets Typ': Typ.
  induction Typ; introv Red; try solve [ inversion Red ].
  - dependent destruction Red.
   ...
   rewrite concat_empty_r. reflexivity.
   rewrite H4 in Typ_chk.
   forwards*  : typing_through_subst_ee_inf1.
   admit.
   rewrite* (@subst_ee_intro y).
   assert (E = E&empty).
   rewrite concat_empty_r. reflexivity.
  ...
  (* case: app *)
   - right. destruct* IHTyp1 as [Val1 | [e1' Rede1']].
    + destruct* IHTyp2 as [Val2 | [e2' Rede2']].
      ...
      destruct H as [V1].
      subst.
      exists* (trm_anno (open_ee e3 (trm_anno e2 S)) V1). apply red_abs.
