...
Lemma typ_red_chk_prev : forall E v A B,
                    value v ->
                    okt E ->
                    typing E v chk B ->
                    typ_red v A ->
...
- assert (sub typ_bot typ_bot) by auto.
  lets : btm_inf_value_false H H0 H3.
  inversion H4.
- auto.
- apply subsub_arrow. auto.
  admit.
...
Lemma subst_btm_false : forall E F x U e u T1 T2,
value u ->
typing (E & x ~: U & F) e inf (typ_arrow T1 T2) ->
typing (E & F) (subst_ee x u e) inf typ_bot -> False.
Proof.
intros.
inductions e.
inversion H0.
...
Lemma subst_btm_false1 : forall E F x U e u T1 T2,
value u ->
typing (E & x ~: U & F) e inf (typ_arrow T1 T2) ->
typing (E & F) (subst_ee x u e) inf typ_bot -> False.
Proof.
...
Lemma subst_union_false : forall E F x U e u T1 T2 t1 t2,
value u ->
typing (E & x ~: U & F) e inf (typ_arrow T1 T2) ->
typing (E & F) (subst_ee x u e) inf (typ_or t1 t2) -> False.
Proof.
intros.
inductions H.
 - inductions e; try solve [inversion H0].
   admit.
   inversion H0; subst.
   inversion H1.
   inversion H0; subst.
...
Lemma typing_through_subst_ee_inf_chk : forall E F x T e u dir U U',
   typing (E & x ~: U & F) e dir T ->
   typing E u inf U' ->
   sub U' U ->
   value u ->
...
Proof.
  ...
  - dependent destruction Red.
   ...
   + 
     lets Typ2': Typ.
     apply typing_regular in Typ2'. destruct Typ2'.
     forwards * : typ_red_chk_prev Typ H6.
...
Definition preservation_sub := forall E e e' dir T,
  typing E e dir T -> 
  red e e' -> 
  exists T', typing E e' dir T' /\ sub T' T.

Lemma preservation_result_sub : preservation_sub.
Proof.
  unfold preservation_sub.
  introv Typ. gen e'.
  lets Typ': Typ.
  induction Typ; introv Red; try solve [ inversion Red ].
  - dependent destruction Red.
  ...
  - dependent destruction Red.
   ...
   + apply typing_regular in Typ.
     destruct~ Typ.
     inversion Typ'; subst.
     eapply typing_sub_inf in H4; eauto.
  - dependent destruction Red.
   + forwards * : IHTyp.
    destruct H5 as [S].
    destruct H5.   
...
Lemma typing_through_subst_ee_inf_empty : forall x T e u dir U U',
   typing (empty & x ~: U & empty) e dir T ->
   typing empty u inf U' ->
   sub U' U ->
   value u ->
   exists T', typing empty (subst_ee x u e) dir T' /\ sub T' T.
Proof.
introv TypT TypU subsub valu.
lets TypT': TypT. 
inductions TypT; introv; simpl.
- case_var.
 + 
   binds_get H0.
   lets M: (@typing_weakening empty empty empty u inf U').
   do 2 rewrite concat_empty_r in M.
   exists U'. split.
   apply* M. auto.
 + exists T. split*.
...
Definition preservation_sub_nil := forall e e' dir T,
  typing empty e dir T -> 
  red e e' -> 
  exists T', typing empty e' dir T' /\ sub T' T.

Lemma preservation_result_sub_nil : preservation_sub_nil.
Proof.
  unfold preservation_sub_nil.
  introv Typ. gen e'.
  lets Typ': Typ.
  inductions Typ; introv Red; try solve [ inversion Red ].
  - dependent destruction Red.
   + forwards * : IHTyp1.
    destruct H0 as [S].
    destruct H0.
   ...
   pick_fresh y.
   forwards~ Typ_chk: H3 y.
   assert (empty & y ~: T1 = y ~: T1).
   rewrite concat_empty_l. reflexivity.
   rewrite H6 in Typ_chk.
   forwards*  : typing_through_subst_ee_inf_empty.
   destruct H7. destruct H7.
   rewrite* (@subst_ee_intro y).
   eapply typing_chk_sub; eauto.
   - forwards * : IHTyp.
