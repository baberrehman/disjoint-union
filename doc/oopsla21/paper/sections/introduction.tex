...
Disjointness ensures there it is always
...
\bruno{Probably it should be noted that there's an extension that does
allow some overlap in Ceylon?.}
\ningning{Agree. Probably as a footnote? And say something bad about that extension.}
...
\snow{I think the above statement is not precise. Disjointness is
defined, although the OOPSLA paper has many variants. For example in Fig.14
and Fig.10 dsj is defined on the literal subtyping. For the semantics,
I feel they are hinting for some reduction rules on page 4 ``Flow-Sensitivity",
but we can say no operational semantics definition is given.}
...
\snow{Frankly speaking, only three rules among them applies to our calculi
and they are roughly equal to our algorithmic disjointness.}
