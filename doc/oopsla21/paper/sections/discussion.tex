...
\baber{I think perhaps we can drop this section and mention in the conclusion
just in few lines that \cal provides a formal definition of disjointntess 
whereas ceylon has textual description?}
\ningning{I agree this section feels quite weak for now.}

\bruno{I think we can move this discussion to the Discussion section. } \baber{I agree.}
...
(3) is a straightforward rule for union disjointness (\rref{ad-orl}\bruno{add the rule here}
\baber{Done.}) in \cal.
...
\baber{I think these points are not worth mentioning after Snow's discovery that these bugs
       in Ceylon are because of alias? For now, only point that we argue about in this section is
       that we provide a formal disjointness definition. I agree, this may not be a very strong argument.
       But I think it adds some difference, for example formal definitions are easy to reason about 
       (as mentioned above). And perhaps we can mention it is relatively easy to understand and extend
       formal definition? And should we state again that our disjointness definition (with intersection
       types) is much simpler? I think we can also mention that Ceylon uses disjuntive normal
       form which poses a restriction on how one creates types?}
...
returns empty set ($\{\}$) and \rref{s-los} makes all such types with $[[findsubtypes A]]$ = $\{\}$ as
...
This is again a trivial property (\Cref{lemma:discussion:disjoint-types}) in \cal 
with the novel disjointness definition and \rref{s-los}.

\begin{lemma}[Disjont Types Intersection Bottom-Like]
  If $[[A *s B]]$ then $[[A /\ B <: Bot]]$.
\label{lemma:discussion:disjoint-types}
\end{lemma}

\noindent The use of \rref{s-los} instead of \rref{s-disj} also has the advantage that it does not
...
remain sound and complete.} \baber{Done.}
...
\Cref{sec:inter}. In this section we add distributive subtyping in the variant of \cal with
intersection types. 
...
\emph{do}. Because our standard subtyping relation in \Cref{sec:union,sec:inter}
...
\noindent Particularly, \rref{ds-distor} allows this specific program to type-check.
