...
\ningning{I feel something is missing here. Shall we say explicitly here that
  while Ceylon does type-directed elimination, there is no formalism and we wish
  to study the feature formally and precisely? It now feels like Ceylon has
  already done all the work and it's unclear what's new in this work.}
...
\snow{We didn't explain why lambdas have two annotation here. To select the right
  branch, we don't need the dynamic type annotation of lambda. Moreover, it
  keeps the preservation in beta reduction. Following are two example:}
...
\snow{The first example shows if we don't add type annotation to the input for subsitutition
we will reach ill-typed term.
The second example shows we have to use the dynamic type instead of the static type of lambda.
The expression $e [x->1:Int]$ was legal when x has type Top but becomes illegal when x has type Int.}
