...
elimination of union types, but in a language independent way.
...
if the only supertypes that they can have are top-like types (i.e.
...
(dynamic) type that $[[x]]$ can have, it is always unambigous which branch to choose.
...
\begin{definition}\label{def:disjointness}
	A $*_s$ B $\Coloneqq$ $\forall$ C, $[[C <: A]]$ and $[[C <: B]]$ $\rightarrow$ $[[botlike C]]$
...
\subsubsection*{Disjointness in the presence on intersection types.}
...
disjointness in terms of ordinary types~\cite{davies2000intersection}, which are basically primitive types
...
\subsubsection*{The role of type annotations in the dynamic semantics.}
