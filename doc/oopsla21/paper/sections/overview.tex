...
A switch expression in Ceylon examine a term of a union type,
and eliminates the union in a type-directed way.
It can have multiple branches, each for a different type.
When evaluating, the one matches with the input's type is chosen
and executed.
%
\begin{lstlisting}
...
\end{lstlisting}
%
Static type checking ensures the switch construct satisfies two requirements:
the list of cases is exhaustive, and any two cases do not overlap.
...
On the other hand, unlike the previous discussion, Ceylon does not allow two
branches to cover the same type by posing a disjointness restriction.
For example, \lstinline{String} and \lstinline{Integer|Float} are disjoint,
since no value has both \lstinline{String} and \lstinline{Integer}, or both
\lstinline{String} and \lstinline{Float}.

\subsubsection*{As an Alternative to Overloading}
With such a type-directed union elimination, Ceylon is able to
model a form of function overloading.
To demonstrate, we define a function \lstinline{padLeft}.
Its second argument can be either a string or an integer.
Two different implementations for each of them are embedded
in the function.
...
\end{lstlisting}
%
The switch construct has two cases: one add a string to the left of \lstinline{v},
the other calls function \lstinline{space} to generate an string of \lstinline{x} spaces, and then pad it to of \lstinline{v}.
As a consequence of the non-overlapping requirement, overloaded functions must have
disjoint input types.
There is no restriction on their return types since union types can be introduced
implicitly. For example, if the above two cases return \lstinline{String} and
\lstinline{Integer} separately, the \lstinline{padLeft} can have \lstinline{String|Integer} as its return type.
