...
values $[[v]]$, and context $[[G]]$ stay same as in \Cref{sec:union}.
...
only if all their common subtypes are bottom-like. This definition no longer works with the
addition of intersection types. For example, common subtypes of type $[[Int]]$ and
type $[[Bool]]$ now also include $[[Int /\ Bool]]$.
But $[[Int /\ Bool]]$ is not a bottom-like type. So the disjointness definition fails, since
we can always find a common non-bottom-like type for any two (non-bottom-like) types.
...
However we do not adopt this solution in \name with intersection types for two reasons:
...
\item \Rref{s-dis} complicates the system because
  ...
  for disjointness becomes quite challenging to prove.
...
\item Additionaly, the assumption that intersections of disjoint types
  ...
  with disjoint types can be inhabited with values, and considering those
  types bottom-like would lead to a problematic definition of disjointness and the loss
  of determinism.

\end{enumerate}

For those two reasons we adopt a different solution in \name. 
...
the alternative definition of disjointness presented in Section~\ref{sec:union}.
...
\Cref{def:inter:disj} on few concrete examples:
...
formulation of disjointness. Attempting to adapt the algorithmic formulation
...
The typing and operational semantics are also the same as in \Cref{sec:union}.
\cal with intersection types is type-sound and deterministic. We state the
key lemmas next: \bruno{add determinism}
...
\Cref{inter:list:ceylon} shows a simple Ceylon disjointness example.
...
\noindent The function \emph{speak} in \Cref{inter:list:ceylon} 
