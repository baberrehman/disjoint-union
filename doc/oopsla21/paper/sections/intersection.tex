...
is because of the addition of intersection types $[[A/\ B]]$.
Expressions $[[e]]$, pre-values $[[p]]$, annotated values $[[w]]$,
values $[[v]]$, and context $[[G]]$ stay same as in \Cref{sec:union}. Typing
...
perhaps.} \baber{I think there is no challenge involve in semantics of \cal with
intersection types. It is same as in \cal.}
...
disjoint types to be subtype of $[[Bot]]$ type as by \cite{muehlboeck2018empowering}:

\begin{center}
\drule[]{s-dis}
\end{center}

\noindent \Rref{s-dis} states that insersection type $[[A /\ B]]$
of two disjoint types $[[A]]$ and $[[B]]$ is subtype of $[[Bot]]$ type.
\Rref{s-dis} also complicates the system because
it adds mutual dependency of subtyping and disjointness
which is even harder to tackle and makes completeness
impossible to prove in \cal.
All approaches discussed above add significant complexity in the
system even if they work. We discuss a simple disjointness
approach next which is sound and complete.
...
if they do not have any common \emph{lowest ordinary subtype}. That is the
...
(3) is a straightfard rule for union disjointness in \cal.