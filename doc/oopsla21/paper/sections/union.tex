...
kinds of values: annotated values $w$; and unannotated lambda expressions ($[[\x.e]]$).
In annotated values, which have the form $p :A$, the type annotation $A$
represents the dynamic type that the value has at runtime. The dynamic and static types
are important for the type-directed dynamic semantics.
...
Values which are unannotated lambda expressions ($[[\x.e]]$) are needed
in our calculus because we employ bi-directional type-checking. In bi-directional
type-checking unannotated lambda expressions can be arguments of functions, and unannotated lambda values
will appear as a result of reducing such arguments.
...
$[[A]]$, as stated in \rref{s-ora}. \Rref{s-orb, s-orc} state that if a
...
because the calculus is deterministic this implies that only one of the \rref{Step-Switchl,Step-Switchr}
