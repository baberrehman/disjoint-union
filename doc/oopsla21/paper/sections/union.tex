...
The definition of beta value ($[[betavalue v A B]]$) is shown at the lower part in
\Cref{fig:union:os}.
...
%
Assuming we did not keep the dynamic type of lambdas, here we will not be able
to know $[[A1]]$. And only using $[[A2]]$ may lead to ill-typed programs, like
...
\]
%
We assume the lambda that contains a switch expression that anaylzes the function input.
Say $[[e]]$ is $[[switch x Char e1 Bool e2]]$.
